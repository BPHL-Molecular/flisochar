...
Florida Isolate Characterization Pipeline using hybrid-assembly. Started 2022-08-22.
 ...
 https://github.com/BPHL-Molecular/flisochar
 #### Author
 Tassy J. Bazile <tassy.bazile@flhealth.gov> - https://github.com/BPHL-Molecular
----------------------------------------------------------------------------------------
*/
...
def helpMessage() {
   ...
   Under conditions:
        --genomeSize    genome size when required, used with canu assembler 
        <<
        """.stripIndent()
...
process longqc {
    ...
    */
    ...
    path("${sample_id}.longqc.json"), emit: json
    script:
    """
    mkdir longqc_out
    longQC.py sampleqc -x ont-rapid --sample_name ${sample_id} -o longqc_out/${sample_id} ${datasetFile} --trim_output longqc_out/${sample_id}_trimmed.fastq.gz --ncpu ${threads}
...
log.info" Hybrid Genome Assembly using $params.asb_tool "

if (params.asb_tool == 'dragonflye') {

    process hb_dragonflye {
        tag { sample_id }
        publishDir "$params.outdir/assemblies/dflye", mode: 'copy'
        input:
	...
	"""
	mkdir -p dflye_out
        dragonflye --reads ${lr} --gsize 3.2M --outdir ${sample_id} --polypolish 2 --R1 ${join_l[0]} --R2 ${join_l[1]} --cpus ${task.cpus} --force
...
if (params.asb_tool == 'canu') {
    ...
    // Polishing Canu assembly using Pilon
    process pilon {
        tag { sample_id }
        ...
        export _JAVA_OPTIONS="-Xms64g -Xmx96g" 
        pilon --genome ${can_asb} --bam ${alnbam} --outdir ${sample_id}
...
process quast {
    ...
    script:
    """
    quast $scaffolds -o quast_results_out
    """
 }
...
process kaiju {
    publishDir "$params.outdir/species-identification/kaiju_out",
    mode: 'copy'
    tag { sample_id }
    input:
    tuple val(sample_id), path(any_asb)
    output:
    tuple val(sample_id), path("${sample_id}_kaiju_summary_esp.tsv")

    script:
    """
    kaiju -z ${task.cpus} -t /data/reference/kaiju/20200525/kaiju_db_refseq/nodes.dmp -f /data/reference/kaiju/20200525/kaiju_db_refseq/kaiju_db_refseq.fmi -i ${any_asb} -o ${sample_id}_kaiju.out -v

    kaiju2table -t /data/reference/kaiju/20200525/kaiju_db_refseq/nodes.dmp -n /data/reference/kaiju/20200525/kaiju_db_refseq/names.dmp -r species -o ${sample_id}_kaiju_summary_esp.tsv ${sample_id}_kaiju.out
    """
}
...
// Grouping all Kaiju report files into a single dir
...
publishDir "$params.outdir/species-identification/kaijuSummary_out",
        mode: 'copy'
    input:
    path(kaiju_grouped)
    ...
    script:
    """
    Kaiju_summary_v01.py --kjreppath ${kaiju_grouped}
    """
}
...
process bakta {
    ...
    output:
    path("bakta_output/${sample_id}_bakta")

    script:
    """
    mkdir bakta_output

    bakta --db /data/reference/bakta/db/ --prefix ${sample_id} --output bakta_output/${sample_id}_bakta --threads ${task.cpus} ${any_asb}
    """
}  

// Adding all bakta results into one dir first

process bakta_summary {
    publishDir "$params.outdir/annotation/bakta_summary", mode: 'copy'
    ...
    script:
    """
    bakta_summary_v01.py --bkreppath ${tx}
    """
}
...
process pgap_antn {
    ...
    script:
    """
    pgap_run_v2.py --ppgroup_dir $asb_met_dir
    """
}
...
process ani {
    ...
    script:
    """
    pgap_tax_v1.py --ppgroup_dir ${asb_met_dir}
    """
}
...
workflow {
    // Quality control
    longqc( lreads_ch, threads )
    fastp( sreads_ch )
    lqc_ch = longqc.out.lreads
    sqc_ch = fastp.out.sreads
    lsch_join = sqc_ch.join(lqc_ch) // Joining long reads and short reads channel
